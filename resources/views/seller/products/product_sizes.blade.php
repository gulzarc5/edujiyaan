                            ...
                            <div class="well" style="overflow: auto">
                                <div class="form-row mb-10" id="inner_size_add_div'+key+'">
                                  ...
                                  <div class="col-md-8 col-sm-12 col-xs-12 mb-3">
                                    ...
                                    @if($size->status == 1)
                                      <a href="{{ url('Seller/Products/Size/Status/'.encrypt($size->id).'/'.encrypt(2).'/'.encrypt($size->product_id).'') }}" class="btn btn-sm btn-warning" style="margin-top: 25px;">Disable</a>
                                    @else
                                      <a href="{{ url('Seller/Products/Size/Status/'.encrypt($size->id).'/'.encrypt(1).'/'.encrypt($size->product_id).'') }}" class="btn btn-sm btn-success" style="margin-top: 25px;">Enable</a>                                       
                                    @endif
          ...
          <div class="x_panel">
              ...
              <div>
                  <div class="x_content">
                    ...
                    {{ Form::open(['method' => 'post','route'=>'seller.product_new_size_add']) }}
  ...
  @section('script')
     <script type="text/javascript">
      ...
      function size_save(id) {
        ...
        $.ajax({
            type:"POST",
            url:"{{ route('seller.product_size_update')}}",
            data:{ 
              "_token": "{{ csrf_token() }}",
              size_id:size_id, 
              size:size, 
