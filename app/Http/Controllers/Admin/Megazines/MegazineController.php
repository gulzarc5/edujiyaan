...
namespace App\Http\Controllers\Admin\Megazines;
...
class MegazineController extends Controller
{
    public function addMegazineForm()
    {
    	...
    	return view('admin.megazines.add_megazine_form',compact('magazine_category'));
    }
    ...
    {
        return view('admin.megazines.megazine_list');
    }
    ...
    public function ajaxMegazineList()
    {
        ...
        ->orderBy('megazines.id','desc');
            ...
            ->addColumn('action', function($row){
                   $btn = '
                   <a href="'.route('admin.megazine_detail_view',['megazine_id'=>encrypt($row->id)]).'" class="btn btn-info btn-sm" target="_blank">View</a>
                   <a href="'.route('admin.edit_megazine_form',['book_id'=>encrypt($row->id)]).'" class="btn btn-warning btn-sm">Edit</a>                 
                   ';
                   if ($row->status == '1') {
                       $btn .= '<a href="'.route('admin.megazine_status_update',['megazine_id'=>encrypt($row->id),'status' => encrypt(2)]).'" class="btn btn-danger btn-sm">Disable</a>';
                        return $btn;
                    }else{
                       $btn .= '<a href="'.route('admin.megazine_status_update',['megazine_id'=>encrypt($row->id),'status' => encrypt(1)]).'" class="btn btn-success btn-sm">Enable</a>';
                        return $btn;
                    }
                    return $btn;
            })
    ...
    public function megazineDetailView($megazine_id)
    {
        ...
        return view('admin.megazines.megazine_details',compact('megazine', 'seller'));
    }

    public function coverImageView ($file_name) {

        $path = public_path('\images\megazines\\'.$file_name);
        if (!File::exists($path)) 
            $response = 404;
        $file = File::get($path);
    ...
    public function megazineFileView ($file_name) {

        $path = storage_path('\app\files\megazines\\'.$file_name);
        if (!File::exists($path)) 
            $response = 404;
        $file = File::get($path);
        $type = File::extension($path);
    ...
    public function editMegazineForm($megazine_id)
    {
        ...
        return view('admin.megazines.edit_megazine',compact('magazine_category','megazine'));
    }
